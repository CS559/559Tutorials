 ...
 * edited by Gleicher (Jan 25) to work in Tutorials
 */
// @ts-check
...
if (!(canvas0 instanceof HTMLCanvasElement))
  throw new Error("Canvas is not HTML Element");
...

{
  ...
  let update = function () {
    ...
    clearCanvases([canvas1,canvas2,canvas0])
    
    plotCurve(canvas1, innerPoints, new LagrangePolyCurve(innerPoints), 200);
    let ccurve = new CardinalCurve(thePoints, 0.5)
    plotCurve(canvas2, thePoints, ccurve,200);
    plotCurve(canvas0, thePoints, ccurve,0);
  };
  
  draggablePoints(canvas0, thePoints, update);
